...
export default function FoundItemRegistration1({ navigation }) {
  const [foundImageUrl, setFoundImageUrl] = useState(null);
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setFoundImageUrl(result.assets[0].uri);
  ...
  const handleNoImage = () => {
    setNoImage(!noImage);
    if (!noImage) {
      setFoundImageUrl(null); // "올릴 수 있는 사진이 없어요"를 선택하면 업로드된 사진 초기화
    }
  };

  const handleNext = () => {

    const FoundItemData1 = {foundImageUrl};

    if (foundImageUrl || noImage) {
      navigation.navigate('FoundItemRegistration2', { FoundItemData1 });
      console.log('FoundItemData1 :', FoundItemData1);
    } else {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>어떤 물건을 찾으셨나요?</Text>
  ...
  {foundImageUrl ? (
    <Image source={{ uri: foundImageUrl }} style={styles.image} />
  ) : (
